...
Strain2D LS_strain(const Disp2D &disp, PERSPECTIVE perspective_type, double units_per_pixel, SUBREGION subregion_type, ROI2D::difference_type r) { 
	...
	Array2D<double> dux(disp.data_height(), disp.data_width());
	Array2D<double> duy(disp.data_height(), disp.data_width());
	Array2D<double> dvx(disp.data_height(), disp.data_width());
	Array2D<double> dvy(disp.data_height(), disp.data_width());
    ...
    // Must form union with valid points     
    return { std::move(A_eyy), std::move(A_exy), std::move(A_exx), std::move(dvy), std::move(dvx), std::move(duy), std::move(dux), disp.get_roi().form_union(A_vp), disp.get_scalefactor() };
}
